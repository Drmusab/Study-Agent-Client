...
Continuation, 2026-09-25: the ledger rejects illegal or stale transitions before replacing the snapshot, freezes commit semantics onto the record at prepare, and keeps a tombstone when an old `COMMITTED` payload is compacted. Unresolved rows are not pruned. `ABANDONED` is a timestamp, not a state and not proof the scheduler did nothing. A provider write already in flight is ambiguous, not a safe retry. `rate_card` may include optional `review_commit_id` (omitted when absent). Advertising `review_commit_idempotency` from the client hello is not server support; the mock ignores the field. Automatic rating replay stays off unless that transaction's frozen semantics advertise idempotent replay. This is still not exactly-once.
