...
| text selection | **enabled** (the Chromium long-press default, left alone deliberately) | a reviewer must be able to copy a dose, a gene name or an Arabic term out of a card rather than transcribing it by hand. Disabling it would need a product reason; there is none (STEP 141) |
| selection menu | `disabledActionModeMenuItems = MENU_ITEM_WEB_SEARCH` | "Web search" would hand the selected card text to a third-party search engine — card content leaving the app under a gesture the user reads as local. Copy / Select all / Share / process-text remain; none of them can navigate (STEP 141) |
| context menu | `registerForContextMenu` **never called** | the generic browser menu (open in new tab, download image, save link as) simply does not exist here. Long press belongs to text selection, and `isLongClickable = true` routes it there; a link under a long press still resolves through `shouldOverrideUrlLoading`, so the link policy stays the card's only navigation path (STEP 142, INV-16) |
...
**228 tests: 196 JVM + 32 instrumented.**
...
| `AnkiRendererIsolationTest` | 27 | the renderer exists where the architecture says (and fixtures are debug-only); **core purity** — no Android/Compose/ui/data/BuildConfig imports, no calendar types, and an explicit allowlist of the six non-project imports the core may use; **no bridge** — 12 forbidden tokens, no `evaluateJavascript`, no `loadUrl(…)`, documents arrive through `loadDataWithBaseURL` only; **no scheduler/gateway/voice/AI vocabulary** — 19 + 16 forbidden tokens, no collection writes, and a project-import allowlist; night mode is never a forced inversion; the WebView host pins all 17 required settings; text selection is bounded (web search disabled) and no browser context menu is ever registered; file/content origins refused; TLS cancelled never proceeded; `onRenderProcessGone` handled and returns `true`; navigation goes through the controller; permissions denied and dialogs suppressed; only two files may name `android.webkit` (and the fallback view may not); logging through `AppLogger` only; the surface seam has **no imports at all**; one surface per composition, destroyed on dispose; the entry point does not recreate a WebView on recomposition; no AnkiDroid assets, MathJax or jQuery; the base URL is the reserved origin |
...
| CI (`android-ci.yml`) | **BLOCKED — billing, not code.** `gh` authentication works in this sandbox, and the run history was read directly: **every** workflow run on `master`, on the GATE 08 branch and on the nightly schedule is `failure`, but each job reports *"The job was not started because your account is locked due to a billing issue."* No job has executed, so CI supplies **no** test evidence in either direction. Runs inspected: `35975420454` (nightly, all 6 jobs), `35905867652` (the GATE 08 merge to `master`), `35901373994` / `35901313535` (this branch), plus the GATE 07 runs. Resolving the GitHub Actions billing lock is a prerequisite for any green-build claim — including the earlier gates' |
...
**No unavailable test is reported as PASS.** The 228 tests are committed as executable
specifications: 196 of them need only a JVM and will run the moment a JDK exists; the 32
...
must be executed the moment the environment allows — the 196 JVM tests first, then the 32
